...
/* Función createUser: Guía al usuario en la creación de una contraseña (mínimo 6 caracteres).
El usuario tiene 3 oportunidades para crear y 3 oportunidades para reingresar exitosamente la contraseña .
Función declarativa - Condicional If-Else - Bucle While
*/
function createPassword() {
  let createPasswordSuccess = false;
  let password = prompt(
    "Por favor ingresa una contraseña (mínimo 6 caracteres):"
  );
  ...
  if (password !== null && password !== undefined) {
    while (password.length < 6 && attempts < 3) {
      password = prompt(
        "La contraseña debe tener mínimo 6 caracteres. Por favor ingresa una contraseña más larga:"
      );
      attempts++;
    }
    if (attempts < 3) {
      ...
      if (password !== null && password !== undefined) {
        ...
        if (attempts < 3) {
          ...
          alert(
            "Parece que hoy no estás inspirado. Vuelve a iniciar la aplicación para intentar nuevamente."
          );
        }
      } else {
        alert(
  ...
  console.log("createPasswordSuccess: " + createPasswordSuccess);
...
function createUser() {
  ...
  user = formatString(
    prompt(
      "Primero crearemos tus credenciales de ingreso.\nPor favor ingresa un nombre de usuario (mínimo 3 caracteres):"
    )
  );
  console.log("user: " + user);
  let attempts = 1;

  while (user !== undefined && user.length < 3 && attempts < 3) {
    user = formatString(
      prompt(
        "El usuario debe tener mínimo 3 caracteres. Por favor ingresa un nombre más largo:"
      )
    );
    attempts++;
  }
...
function setWallets() {
  ...
  let keepWallets = [];
  wallets.forEach((wallet) => {
    let keepWallet = confirm(
      "¿Desea conservar la billetera " +
        wallet +
  ...
  msg =
    "¡Muy bien " +
    user +
    "! Estas son las billeteras que dicidiste conservar.\n";
  wallets.forEach((wallet) => {
    msg += " - " + wallet + "\n";
  });
  msg += "\nA continuación podrás agregar otras billeteras";
  alert(msg);
  let newWallet = prompt(
    "Por favor ingrese el nombre de la billetera que desea agregar. \nSi no desea agregar otra billetera presione Cancelar"
  );
...
function setIncomeCategories() {
  let msg =
    user + ", es momento de personalizar las categorías de tus ingresos.\n";
  ...
  for (let index = 0; index < incomeCategories.length; index++) {
    let keepCategory = confirm(
      "¿Desea conservar la categoría " +
  ...
  msg =
    "¡Muy bien " +
    user +
    "! Estas son las categorías de ingresos que dicidiste conservar.\n";
  incomeCategories.forEach((category) => {
    msg += " - " + category + "\n";
  });
...
function setExpensesCategories() {
  let msg =
    user + ", es momento de personalizar las categorías de tus gastos.\n";
  msg += "Te mostramos las categorías predefinidas por la aplicación: \n\n";
  ...
  msg =
    "¡Muy bien " +
    user +
    "! Estas son las categorías de gastos que dicidiste conservar.\n";
  expensesCategories.forEach((category) => {
    msg += " - " + category + "\n";
  });
  msg += "\nA continuación podrás agregar nuevas categorías";
...
function main() {
  ...
  msg +=
    "Te invitamos a recorrer este pequeño tutorial que te ayudará a configurar la aplicación para que se ajuste a tus características financieras.";
  alert(msg);
  if (createUser()) {
    setWallets();
    setIncomeCategories();
    ...
    msg +=
      "En la próxima preentrega podrás registrar los movimientos financieros que realizas diariamente.\n";
    alert(msg);
  }
}
